#!/usr/bin/env python3
"""THERMAL-AUDIT-001 Round 2 — ChatGPT"""
from playwright.sync_api import sync_playwright
from pathlib import Path
from datetime import datetime

PROMPT_FILE = Path.home()/'Documents/NEVA/audit_responses/THERMAL-001-R2-PROMPT.txt'
OUT = Path.home()/'Documents/NEVA/audit_responses/THERMAL-001-R2-CHATGPT.md'
PROMPT = PROMPT_FILE.read_text()
XML = f'<user_attachments><attachment name="THERMAL-001-R2.md">{PROMPT}</attachment></user_attachments>'
...
with sync_playwright() as pw:
    ...
    OUT.write_text(f'# ChatGPT — THERMAL-001-R2\nДата: {datetime.now()}\n\n{text}')
    print(f'Saved: {OUT}')
    p.close()
