#!/usr/bin/env python3
"""
NEVA-TASK-008 — Thermal Guard v10
...
# NSProcessInfoThermalState → виртуальные °C для FSM
# Маппинг намеренно безопасный: виртуальные значения ниже реальных порогов HOT/CRITICAL.
# Nominal(0)=30 < WARM(75) → NOMINAL; Fair(1)=60 < HOT(83) → WARM/NOMINAL;
# Serious(2)=80 < CRITICAL(90) → WARM/HOT_PENDING; Critical(3)=95 ≥ CRITICAL → CRITICAL.
# NSProcessInfo НЕ заменяет точную температуру — только выводит систему из DEGRADED.
_NS_THERMAL_MAP = {0: 'NOMINAL', 1: 'WARM', 2: 'DEGRADED', 3: 'CRITICAL'}
...
class ThermalGuard:
    ...
    def _read_temp_nsprocessinfo(self):
        """NSProcessInfo.thermalState() → виртуальная температура или None.
        Не требует sudo. Выводит систему из DEGRADED при отсутствии powermetrics.
        ImportError уже обработан на уровне модуля (_HAS_PYOBJC=False).
        """
        if not _HAS_PYOBJC:
            return None
        try:
            state = int(_NSProcessInfo.processInfo().thermalState())
            virtual = NS_VIRTUAL_TEMP.get(state)
            if virtual is not None:
                self.logger.debug(f"NSProcessInfo thermalState={state} → virtual={virtual}°C")
            return virtual
        except Exception as e:
    ...
    def _read_temp(self):
        """Каскад: powermetrics → NSProcessInfo → None (fallback на swap/mem).
        Возвращает (float|None, is_stale: bool, source: str)"""
        # 1. powermetrics (точная температура, требует sudo)
        ...
        if p.is_alive():
            ...
            if result[0] == 'ok' and result[1] is not None:
                ...
                q.close(); q.join_thread() if not q._closed else None
        else:
            q.close(); q.join_thread()

        self.state['powermetrics_available'] = False

        # 2. NSProcessInfo (без sudo, 4 уровня)
        virtual = self._read_temp_nsprocessinfo()
        if virtual is not None:
            return virtual, False, 'nsprocessinfo'

        # 3. Fallback — нет температуры, FSM использует swap/mem
