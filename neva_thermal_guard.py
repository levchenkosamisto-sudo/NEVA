#!/usr/bin/env python3
"""
NEVA-TASK-008 — Thermal Guard v11
...
Директор: Серж
v9 утверждено: 2026-06-06
v10 изменения (аудит THERMAL-001, 2026-06-23):
  A. _read_temp(): каскад powermetrics → NSProcessInfo → fallback.
  B. _load_state(): fcntl.LOCK_SH при чтении — fix race condition.
  C. _notify_medic_uds(): push CRITICAL/BLOCKED → Medic через UDS.
v11 изменения (аудит THERMAL-001 R3→R4, 2026-06-23):
  A2. _determine_level(): при source='nsprocessinfo' использует отдельный
      путь FSM по категориям Apple (Nominal/Fair/Serious/Critical)
      БЕЗ подстановки виртуальных °C в числовые пороги.
      Виртуальные числа из NS_VIRTUAL_TEMP удалены из логики FSM.
      Используются только для логирования temp_c в state.
"""
...
# ─── PyObjC для NSProcessInfo (опционально) ──────────────────────────────────
try:
    from Foundation import NSProcessInfo as _NSProcessInfo
    _HAS_PYOBJC = True
except ImportError:
    _HAS_PYOBJC = False

# NSProcessInfoThermalState → категория угрозы (v11: прямой маппинг без °C в FSM)
# Используется ТОЛЬКО когда powermetrics недоступен.
# 0=Nominal, 1=Fair, 2=Serious, 3=Critical
_NS_LEVEL_MAP = {0: 'NOMINAL', 1: 'WARM', 2: 'DEGRADED', 3: 'CRITICAL'}
# Виртуальные °C только для записи в state['temp_c'] и логов — НЕ для FSM порогов
NS_VIRTUAL_TEMP = {0: 30.0, 1: 60.0, 2: 80.0, 3: 95.0}

# UDS путь к Medic
MEDIC_UDS_PATH = '/tmp/neva_medic.sock'
...
def _run_powermetrics() -> tuple:
    """Запускает powermetrics напрямую через subprocess (без multiprocessing).
    Возвращает (temp: float|None, duration: float, error: str|None).
    v11: заменяет multiprocessing.Process — дочерний Python процесс
    выходил с exit 78 под launchd, блокируя KeepAlive перезапуск.
    """
    if not os.path.exists('/usr/bin/powermetrics'):
        return None, 0.0, 'powermetrics not found'
    ...
    try:
        ...
        if r.returncode != 0:
            return None, duration, f"returncode={r.returncode} {r.stderr[:100]}"
        return parse_max_temp(r.stdout), duration, None
    except subprocess.TimeoutExpired:
        return None, 7.0, 'timeout'
    except PermissionError as e:
        return None, 0.0, f"PermissionError: {e}"
...
class ThermalGuard:
    ...
    def __init__(self, dry_run=False):
        ...
        self.last_temp = None
        self.last_swap = 0.0
        self._last_ns_state = None   # v11: категория NSProcessInfo (0-3 или None)
    ...
    def _load_state(self):
        ...
        try:
            with open(STATE_PATH, encoding='utf-8') as f:
                fcntl.flock(f, fcntl.LOCK_SH)  # v10: shared lock при чтении
                try:
                    state = json.load(f)
                finally:
    ...
    # ─── Температура (v10: каскад источников) ────────────────────────────────
    def _read_temp_nsprocessinfo(self):
        """NSProcessInfo.thermalState() → (ns_state: int, virtual_temp: float) | (None, None).
        ImportError обёрнут на уровне модуля (_HAS_PYOBJC=False).
        Возвращает int-категорию Apple (0-3) + виртуальный °C только для логов.
        """
        if not _HAS_PYOBJC:
            return None, None
        try:
            ns_state = int(_NSProcessInfo.processInfo().thermalState())
            virtual = NS_VIRTUAL_TEMP.get(ns_state)
            self.logger.debug(f"NSProcessInfo thermalState={ns_state} "
                              f"(virtual={virtual}°C, logging only)")
            return ns_state, virtual
        except Exception as e:
            self.logger.warning(f"NSProcessInfo error: {e}")
            return None, None

    def _read_temp(self):
        """Каскад: powermetrics → NSProcessInfo → fallback.
        v11: powermetrics через subprocess.run (не multiprocessing.Process).
        Возвращает (temp: float|None, is_stale: bool, source: str).
        """
        # 1. powermetrics через subprocess напрямую (нет дочернего Python процесса)
        temp, duration, err = _run_powermetrics()
        if err:
            self.logger.error(f"powermetrics error: {err}")
            ...
            return temp, is_stale, 'powermetrics'

        self.state['powermetrics_available'] = False

        # 2. NSProcessInfo (без sudo, 4 уровня Apple)
        ns_state, virtual = self._read_temp_nsprocessinfo()
        if ns_state is not None:
            self._last_ns_state = ns_state
            self.state['temp_c'] = virtual
            return None, False, 'nsprocessinfo'

        # 3. Fallback — только swap/mem
        self._last_ns_state = None
    ...
    # ─── Уровень угрозы (v11: раздельные пути по источнику) ─────────────────
    def _determine_level(self, temp, swap_gb, mem_pct, source='powermetrics'):
        """
        Три режима (v11):
          source='powermetrics'  → реальные °C → числовые пороги (HOT_TEMP_C и т.д.)
          source='nsprocessinfo' → категория Apple (_last_ns_state 0-3) → прямой маппинг
          source='fallback'      → только swap/mem → DEGRADED или DEGRADED_HIGH
        """
        thr = self.config['thresholds']

        # ── Режим NSProcessInfo: прямое отображение без °C ───────────────────
        if source == 'nsprocessinfo' and self._last_ns_state is not None:
            ns = self._last_ns_state
            self.logger.debug(f"FSM via NSProcessInfo: state={ns}")
            if ns == 3:                         # Critical
                self._reset_hot_pending()
                return 'CRITICAL'
            if ns == 2:                         # Serious
                # Проверяем swap/mem для эскалации до DEGRADED_HIGH
                if (swap_gb >= thr['DEGRADED_HIGH_SWAP_GB'] or
                        mem_pct >= thr['DEGRADED_HIGH_MEM_PCT']):
                    return 'DEGRADED_HIGH'
                return 'DEGRADED'
            if ns == 1:                         # Fair
                if (swap_gb >= thr['SWAP_WARN_RISE_GB']):
                    return 'WARM'
                return 'NOMINAL'
            # ns == 0: Nominal
            return 'NOMINAL'

        # ── Режим fallback: только swap/mem ──────────────────────────────────
        if temp is None:
            self.state['powermetrics_available'] = False
            self._reset_hot_pending()
            if (swap_gb >= thr['DEGRADED_HIGH_SWAP_GB'] or
        ...
        # ── Режим powermetrics: реальные °C → числовые пороги ────────────────
        self.state['powermetrics_available'] = True
        ...
        current = self.state['state']
        if current == 'WARM':
            if (temp < thr['WARM_TEMP_C'] - 3 and swap_gb < thr['SWAP_WARN_FALL_GB']):
                return 'NOMINAL'
            return 'WARM'
        else:
            if (temp >= thr['WARM_TEMP_C'] or swap_gb >= thr['SWAP_WARN_RISE_GB']):
                return 'WARM'
        return 'NOMINAL'
    ...
    def _do_critical(self):
        ...
        self._notify_medic_uds('CRITICAL', temp=self.last_temp)  # v10: push→Medic

    def _do_degraded_high(self):
        self.logger.info("DEGRADED_HIGH: выгружаем qwen по давлению памяти")
        self._unload_model('qwen2.5:7b')
        self._transition('BLOCKED')
        self._notify_medic_uds('BLOCKED', temp=self.last_temp)  # v10: push→Medic
    ...
    # ─── UDS push к Medic (v10) ──────────────────────────────────────────────
    def _notify_medic_uds(self, state: str, temp=None):
        """Fire-and-forget push к Medic. Полностью неблокирующий (setblocking=False)."""
        payload = json.dumps({
            'event': 'THERMAL_STATE_CHANGE',
            'state': state,
            'temp': temp,
        ...
        try:
            with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as s:
                s.setblocking(False)  # истинный fire-and-forget (Gemini R3)
                s.connect(MEDIC_UDS_PATH)
                s.sendall(payload)
            self.logger.debug(f"UDS→Medic: {state}")
        except (FileNotFoundError, ConnectionRefusedError, BlockingIOError, OSError):
            pass  # Medic недоступен — работаем автономно
        except Exception as e:
            self.logger.debug(f"UDS push failed: {e}")
    ...
    def run_self_test(self):
        print("=== NEVA Thermal Guard v11 — Self-test ===\n")
        results = {}

        # 1. Каскад источников температуры (v11)
        temp, stale, temp_src = self._read_temp()
        if temp is not None:
            results['1_temp_cascade'] = f'PASS src=powermetrics temp={temp}°C stale={stale}'
        elif temp_src == 'nsprocessinfo' and self._last_ns_state is not None:
            results['1_temp_cascade'] = (
                f'PASS src=nsprocessinfo thermalState={self._last_ns_state} '
                f'(powermetrics требует sudo — нормально)'
            )
        else:
            results['1_temp_cascade'] = 'WARN: все источники недоступны (fallback swap/mem)'
        ...
        # 9. NSProcessInfo (PyObjC)
        try:
            from Foundation import NSProcessInfo
            state = NSProcessInfo.processInfo().thermalState()
            results['9_nsprocessinfo'] = f'PASS (thermalState={state})'
        except ImportError:
            results['9_nsprocessinfo'] = 'WARN: PyObjC не установлен (pip install pyobjc-framework-Foundation)'
        except Exception as e:
            results['9_nsprocessinfo'] = f'FAIL: {e}'
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile('w') as f:
                ...
                fcntl.flock(f, fcntl.LOCK_UN)
            results['10_fcntl_lock'] = 'PASS'
        except Exception as e:
            results['10_fcntl_lock'] = f'FAIL: {e}'

        # 11. UDS connect (fire-and-forget — ожидаем отказ если Medic не запущен)
        try:
            import socket
            s = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
            s.settimeout(0.3)
            s.connect('/tmp/neva_medic.sock')
            s.close()
            results['11_uds_medic'] = 'PASS (Medic слушает UDS)'
        except (FileNotFoundError, ConnectionRefusedError):
            results['11_uds_medic'] = 'WARN: Medic UDS недоступен (нормально если Medic не запущен)'
        except Exception as e:
            results['11_uds_medic'] = f'FAIL: {e}'
        ...
        print()
        if failed:
            print(f"❌ FAIL: {len(failed)} тестов не прошли: {failed}")
            sys.exit(1)
        else:
            w = f", {len(warned)} WARN" if warned else ""
            print(f"✅ PASS: {len(results) - len(warned)} из {len(results)} тестов{w}")

    # ─── Главный цикл ────────────────────────────────────────────────────────
    def run(self):
        self.logger.info("=== NEVA Thermal Guard v11 запущен ===")
        if self.dry_run:
        ...
        while True:
            try:
                ...
                temp, is_stale, temp_src = self._read_temp()
                mem = self._read_memory()
                ...
                level = self._determine_level(
                    temp, mem['swap_gb'], mem['mem_pct'], source=temp_src
                )
                self._act(level)
