...
with sync_playwright() as pw:
    ...
    if 'Войти' in (p.inner_text('body')[:300]): print('[Grok] NOT LOGGED IN'); exit(1)
    try: p.locator('button:has-text("Отклонить")').first.click(timeout=3000)
    except: pass
    el=p.locator('div[contenteditable="true"]').first
    el.click(); p.wait_for_timeout(300); el.fill(text)
    p.keyboard.press('Enter'); print('[Grok] sent')
    ...
    if msgs: r=msgs[-1].inner_text()
    else:
        msgs=p.locator('.message-bubble').all()
        r=msgs[-1].inner_text() if msgs else 'NO RESPONSE'
    r=re.sub(r'Размышление на протяжении \d+s\s*','',r).strip()
    (P/'THERMAL-001-R3-GROK.md').write_text(f'# Grok R3\n{datetime.now()}\n\n{r}')
    print(f'[Grok] {len(r)} chars'); p.close()
