...
class ThermalGuard:
    ...
    def _read_temp(self):
        ...
        Возвращает (temp_real: float|None, is_stale: bool, source: str).
        source='powermetrics'  → temp содержит реальные °C, FSM использует пороги
        source='nsprocessinfo' → temp=None (FSM использует _determine_level_ns)
        source='fallback'      → temp=None (FSM использует только swap/mem)
        """
        # 1. powermetrics (реальная температура, требует sudo)
        ...
        if p.is_alive():
            ...
            result = q.get_nowait(); q.close(); q.join_thread()
            if result[0] == 'ok' and result[1] is not None:
        ...
        # 2. NSProcessInfo — проверяем наличие; если есть, сохраняем ns_state в self
        ns_state, virtual = self._read_temp_nsprocessinfo()
        if ns_state is not None:
            self._last_ns_state = ns_state          # используется в _determine_level
            self.state['temp_c'] = virtual          # только для телеметрии в state
    ...
    def run_self_test(self):
        print("=== NEVA Thermal Guard v10 — Self-test ===\n")
        results = {}
